...
## Where agents and skills fit

Most readers arrive fluent in a vocabulary Bob does not use. Agents,
skills, hosted schedulers, and managed sandboxes all have obvious
places in other stacks. Three of them are positions Bob took on
purpose. One is a question it has not answered.

**Agents.** In the vendor sense, an agent is the model holding the
control flow. It picks the next action, it decides when to delegate,
and it decides when to stop. Bob's one rule forbids exactly that, so
the absence is a position and not a gap waiting to be closed. Bob does
fan out. Orchestra runs parallel actors in `parallel_thinking` and
`council_four`, but at the workflow level, where every invocation
crosses the executor and lands in the log as a checkable, resumable
event. A subagent spawned inside a single CLI session does model work
that never crosses that chokepoint. It is unlogged and unresumable,
and nothing outside the model has checked it.

**Skills.** A skill is static text loaded on demand. It takes no
control flow, so nothing in the thesis rules it out, and Bob still
does not use one. Conventions live in CLAUDE.md instead, which is the
always-loaded version of the same idea. The tradeoff is real and
unresolved. A manifest is paid on every task whether or not it is
relevant, where a skill is paid only when the model reaches for it.
Turning skills on means adding one entry to the allowed-tools lists
that McLoop and Orchestra's agent adapter deliberately keep mirrored.
Nobody has measured whether it helps. The question is open, and it is
written down here so its absence does not read as an oversight.
...
and partly built. It is not yet closed, and the Status section says
where it stands. The boundary holds either way, because it is an architectural
claim and not a status one. Deciding when to run belongs to the
caller. A managed cron does not invoke McLoop at all. It fires
sessions into a vendor's container, solving the problem from outside
the stack and billing per token against the subscription the fallover
...
The same pattern runs through the rest of this file. Bob is not
missing these primitives. It declined most of them deliberately, and
it says out loud which one it did not.
