...
def _render_for_validation(
    ...
    *,
    ...
    Task ``trailing_lines`` (lossless prose and fenced blocks the parser
    captured from disk) pass through validation untouched: the semantic
    normalizers already clear them before comparing, the construction
    API cannot produce them, and the renderer round-trips them
    byte-for-byte, so there is no invariant for them to violate. The old
    "must be empty on constructed tasks" rejection made every runtime
    save destructively strip content that ``fmt`` preserves.
    ...
    if validation == "canonical":
        ...
        # id, duplicate phase ids, task id presence/format, scalar
        # field-stability INCLUDING the embedded-newline preamble check,
        # per-task field-stability) but NOT acceptance-completeness.
...
def save(
    ...
    * ``"unchecked"`` — render with :func:`render_plan` and write the
    ...
    Task ``trailing_lines`` parsed from disk pass through the canonical
    gate and are preserved in the written bytes; see
    :func:`_render_for_validation`.
    """
    if not magic:
