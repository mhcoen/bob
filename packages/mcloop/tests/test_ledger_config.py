...
class TestDefaults:
    def test_default_disabled_when_no_ledger_dir(self, tmp_path: Path) -> None:
    ...
    def test_default_enabled_when_ledger_dir_exists(self, tmp_path: Path) -> None:
...
class TestConfigLayer:
    def test_config_disabled_overrides_autodetect(self, tmp_path: Path) -> None:
        (tmp_path / ".duplo" / "ledger").mkdir(parents=True)
        _write_orchestra_config(tmp_path, {"plan_ledger": {"enabled": False}})
        s = load_plan_ledger_settings(project_dir=tmp_path)
        assert s.enabled is False

    def test_config_auto_reauthor_off(self, tmp_path: Path) -> None:
        _write_orchestra_config(tmp_path, {"plan_ledger": {"auto_reauthor": False}})
        s = load_plan_ledger_settings(project_dir=tmp_path)
        assert s.auto_reauthor is False

    def test_config_custom_ledger_dir(self, tmp_path: Path) -> None:
        custom = tmp_path / "alt-ledger"
        _write_orchestra_config(tmp_path, {"plan_ledger": {"ledger_dir": str(custom)}})
        s = load_plan_ledger_settings(project_dir=tmp_path)
    ...
    def test_config_threshold_params(self, tmp_path: Path) -> None:
        _write_orchestra_config(
            tmp_path,
            {"plan_ledger": {"threshold_params": {"exploratory_commit_limit": 7}}},
        )
        s = load_plan_ledger_settings(project_dir=tmp_path)
...
class TestPrecedence:
    def test_cli_no_plan_ledger_beats_config_enabled(self, tmp_path: Path) -> None:
        _write_orchestra_config(tmp_path, {"plan_ledger": {"enabled": True}})
        s = load_plan_ledger_settings(project_dir=tmp_path, cli_no_plan_ledger=True)
        assert s.enabled is False

    def test_cli_no_auto_reauthor_beats_config(self, tmp_path: Path) -> None:
        _write_orchestra_config(tmp_path, {"plan_ledger": {"auto_reauthor": True}})
        s = load_plan_ledger_settings(project_dir=tmp_path, cli_no_auto_reauthor=True)
        assert s.auto_reauthor is False
