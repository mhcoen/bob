...
2. **Profile registry.** Holds the set of profiles known to the
   ...
   policies), and the conflict-detection logic.
3. **Artifact store.** Holds typed, versioned artifact values for
   ...
   versions only; never mutates an existing version.
4. **Adapter set.** One adapter per registered actor backing.
   ...
   history, subprocess sessions) is managed by the adapter
   internally and reported back to the executor through the
   payload.
5. **Executor.** The state-machine loop. Picks the next state to
   ...
   transition table, and routes to the next state.
...
history management. Reads the agent's running messages artifact
(by name from the artifact store) at `prepare` time, appends the
state's input as a new turn, and submits the resulting
conversation to the underlying model. After invocation, appends
the model's response to the messages artifact (creating a new
version) and reports the new artifact name and version in the
agent payload. Compression-policy enforcement happens here: when
the policy's trigger condition fires, the adapter requests a
compression run from the runner-level compression machinery and
reports the resulting `compression_event` in the payload.
...
coordination happens through artifacts in the artifact store, not
through direct adapter-to-adapter calls.
...
store.write(name, value) -> version_id
...
`write` commits a new version of the artifact and returns the
new version ID. Versions are immutable once committed; existing
...
The `tentative_*` operations support the result parser failure
rollback rule from the result-schemas document. Profile result
parsers produce tentative writes during invocation finalization;
if any parser fails, all tentative writes for that invocation
are discarded. If all parsers succeed and postconditions pass,
the executor commits all tentatives at once. The store
guarantees atomicity at the granularity of one
`commit_tentative` call: either every handle in the list becomes
a committed version, or none does.
...
predicates, result parsers, validation rules), it checks whether
any two loaded profiles registered the same name. If any
collision is found, the workflow is rejected with a load error
naming the conflicting profiles and registrations.
...
send) and returns a payload. The executor records the end time
...
declared writes, the executor runs the parser callback against
the payload. Each parser produces tentative writes for the
artifacts it is responsible for. If any parser fails, the
executor sets `status` to `error`, `outcome` to `error`,
`error.kind` to `parser_failure`, discards all tentative writes
for this invocation, and proceeds to step 8.
...
The runner emits the following event types, in approximate
order of when they appear during a state's execution:
...
- `actor_invoke_end`: emitted at the end of step 4. Includes
  the payload summary (output length and verdict for LLM,
...
- `state_exit`: emitted at step 8. Includes the full envelope
  in a compact form (status, outcome, duration_ms, inputs_read,
  artifacts_written, error). The full payload is referenced by
  the corresponding `actor_invoke_end` record's `seq`, not
  duplicated.
...
- `compression_event`: emitted by the agent adapter when it
  triggers a compression pass. Includes the
...
addition to the state name. The aggregate `state_exit` record
follows once the join policy has been satisfied and the
aggregate envelope has been finalized.
...
interrupted mid-execution. The runner re-enters the same state.
Re-entry increments `attempts.<state>` again, so the next
envelope's `attempt` is one greater than the partial attempt's.
...
modified the workspace will run again. A model call that cost
money will be made again. A human notification that was sent
may already have been answered, in which case re-sending is
inappropriate.
...
- **Model and agent adapters**: re-invoke unconditionally.
  ...
  state.
- **Shell adapters**: re-invoke unconditionally. The
  versioned-workspace profile's checkpoint mechanism is what
  protects the workspace from accumulating partial mutations
  across re-runs; the `git-workspace` artifact's version 0 at
  resume time is the most recent committed checkpoint, not the
  pre-crash workspace state.
...
2. **Compression actor invocation path.** When an agent
   ...
   adapter invokes the runner's compression machinery. Whether
   that machinery is itself a special-case actor (with its own
   adapter and its own envelope), a shared library function
   that adapters call directly, or something in between, is an
