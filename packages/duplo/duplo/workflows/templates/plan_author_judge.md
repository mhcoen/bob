...
assumption that validation will catch structural defects; accept only
when the proposal genuinely satisfies every criterion.
...
Decision semantics:
  ...
  - "accept": the proposal is acceptable as-is given the reviewer's
    critique. The workflow proceeds to canonical validation.

  - "iterate": another review pass would likely surface a clearer
    judgement. The workflow loops back to the proposer with your
    feedback.

  - "stuck": choose "stuck" when the same material issue persists
    after prior feedback, and you assess that further iteration is
