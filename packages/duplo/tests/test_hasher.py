...
def test_load_hashes_corrupt_json_is_preserved(tmp_path: Path) -> None:
    (tmp_path / ".duplo").mkdir()
    (tmp_path / ".duplo" / "file_hashes.json").write_text("{not valid json")
    with pytest.raises(StateError, match="preserved"):
        load_hashes(tmp_path)
    assert (tmp_path / ".duplo" / "file_hashes.json").read_text() == "{not valid json"


def test_load_hashes_binary_file_is_preserved(tmp_path: Path) -> None:
    (tmp_path / ".duplo").mkdir()
    (tmp_path / ".duplo" / "file_hashes.json").write_bytes(b"\xff\xfe\x00\x01\x80")
    with pytest.raises(StateError, match="preserved"):
        load_hashes(tmp_path)
    assert (tmp_path / ".duplo" / "file_hashes.json").read_bytes() == b"\xff\xfe\x00\x01\x80"


def test_load_hashes_unreadable_file_reports_io_error(tmp_path: Path) -> None:
    (tmp_path / ".duplo").mkdir()
