...
class TestRunShScaffoldTemplate:
    ...
    def test_template_does_not_silently_fall_back_to_non_dev_install(self):
        ...
        assert lines, "expected at least one executable pip install -e line in run.sh template"
        for line in lines:
            assert ".[dev]" in line, (
                f"pip install line in run.sh template must request the [dev] extra: {line!r}"
            )
            assert "||" not in line, (
                f"pip install line must not chain a fallback to a non-dev install: {line!r}"
            )
    ...
    def test_template_does_not_redirect_pip_stderr_to_devnull(self):
        ...
        for line in self._executable_install_lines(content):
            assert "2>/dev/null" not in line, (
                f"pip install line must not redirect stderr to /dev/null: {line!r}"
            )
...
class TestRunShWrittenByScaffold:
    ...
    def test_write_scaffold_run_sh_dispatches_test_to_pytest(self, tmp_path: Path):
    ...
    def test_write_scaffold_run_sh_forwards_normal_cli_to_package(self, tmp_path: Path):
    ...
    def test_write_scaffold_run_sh_test_arm_uses_underscore_package(self, tmp_path: Path):
...
class TestProjectNameToPackageName:
    ...
    def test_multi_hyphen_becomes_underscores(self):
        assert project_name_to_package_name("fswatch-run-smoke") == "fswatch_run_smoke"
...
class TestHyphenatedProjectNameScaffold:
    ...
    def test_pyproject_distribution_name_keeps_hyphens(self, tmp_path: Path):
        write_scaffold([_get_profile()], self.PROJECT_NAME, target_dir=tmp_path)
    ...
    def test_package_directory_uses_underscore_name(self, tmp_path: Path):
        ...
        assert not hyphen_dir.exists(), "scaffold must NOT create a hyphenated package directory"
    ...
    def test_underscore_only_name_passes_through_unchanged(self, tmp_path: Path):
        ...
        assert data["project"]["scripts"]["fswatch_run"] == ("fswatch_run.__main__:main")
        assert data["tool"]["setuptools"]["packages"]["find"]["include"] == ["fswatch_run*"]
...
class TestScaffoldPassesValidators:
    ...
    def test_scaffold_pyproject_module_paths_are_python_identifiers(self, tmp_path: Path):
        ...
        for value in data["project"]["scripts"].values():
            ...
            for segment in module_path.split("."):
                assert segment.isidentifier(), (
                    f"script module segment {segment!r} is not a valid Python identifier"
                )
        ...
        for pat in data["tool"]["setuptools"]["packages"]["find"]["include"]:
            ...
            assert stripped.isidentifier(), (
                f"packages.find.include segment {stripped!r} is not a valid Python identifier"
            )
